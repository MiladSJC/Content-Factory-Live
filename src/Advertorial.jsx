...
function Advertorial({ onPushToDAM }) {
    ...
    // Sync orderedImages when inputs/results change
    useEffect(() => {
    ...
    const handleDrop = (e, dropIndex) => {
        ...
        // Ensure array is large enough if dropping into a slot that doesn't exist yet (unlikely in this UI but good safety)
        while (newOrder.length <= Math.max(draggedIndex, dropIndex)) {
            newOrder.push(null);
        }
    ...
    const handleGenerate = async () => {
        ...
        // Step 1: Call LLM to generate article content from creative brief
        try {
            setProcessingStep(0); // Analyzing
            ...
            // Continue with demo fallback even if LLM fails
        }

        // Step 2: Continue with layout generation (demo mode)
        setTimeout(() => setProcessingStep(1), 800); // Designing
        setTimeout(() => setProcessingStep(2), 1600); // Rendering
        setTimeout(() => {
    ...
    const getBrandTone = () => {
        // Fixed values for consistent brand styling
        return {
            bandAlpha: 0.66,
            borderAlpha: 0.64,
            badgeScale: 1.05,
    ...
    // Use orderedImages for preview layout
    ...
    const MagazinePage = ({ pageIndex = 0 }) => {
        ...
        const ImageFrame = ({ src, label, tall = false, index }) => (
            <div
                className="relative overflow-hidden rounded-lg group cursor-move"
                draggable
            ...
            >
                <div className={tall ? 'aspect-[5/6]' : 'aspect-[16/9]'}>
                    {src ? (
                        <img src={src} className="w-full h-full object-cover transition-all duration-500" alt="" />
                    ) : (
                    ...
                    <div className="grid" style={{ gridTemplateColumns: showImages && !isMinimal ? '1.1fr 0.9fr' : '1fr', gap: 18, alignItems: 'start' }}>
                        ...
                        {showImages && !isMinimal && (
                            <div className="space-y-2">
                                <ImageFrame src={heroImage} label="Hero" index={0} />
                            </div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {columnCount >= 1 && (
                            <div className="mt-4 grid" style={{ gridTemplateColumns: columnCount === 3 ? '1fr 1fr 1fr' : (columnCount === 2 ? '1fr 1fr' : '1fr'), gap: 10 }}>
                                <ImageFrame src={secondaryImage} label="Detail" tall={columnCount === 3} index={1} />
                                {columnCount >= 2 && <ImageFrame src={tertiaryImage} label="Texture" tall={columnCount === 3} index={2} />}
                                {columnCount === 3 && <ImageFrame src={orderedImages[3]} label="Alt" tall index={3} />}
                            </div>
                        )}
                    </div>

                    {/* Bottom Signature Bar removed */}
                </div>
    ...
    const MagazinePreview = () => {
        ...
        useEffect(() => {
            const handleResize = (entries) => {
                ...
                // Target dimensions: Layout width (~680px) + minimal padding
                const targetW = 750;
                const targetH = 950;

                const scaleW = width / targetW;
                const scaleH = height / targetH;
                const scale = Math.min(scaleW, scaleH, 1) * 0.95; // 0.95 safety margin
                setScale(scale);
            };
            ...
            >
                <div
                    ...
                    style={{
                        ...
                        width: '680px' // Fixed base width for calculation
                    }}
                >
            ...
            <div className="lg:col-span-1 bg-gray-800 rounded-lg p-5 overflow-y-auto space-y-5 border border-gray-700 shadow-xl custom-scrollbar">
                ...
                <div className="grid grid-cols-3 gap-2">
                    ...
                    <div>
                        ...
                        <div className="w-full bg-gray-900 border border-gray-700 rounded py-1.5 px-2 text-[10px] font-bold text-white">
                            Full Page
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-between items-center mb-3">
                    ...
                    {resultImages.length > 0 && !isProcessing && (
                        <div className="flex gap-2">
                            ...
                            <button
                                onClick={() => onPushToDAM({ path: orderedImages[0] || resultImages[0], name: `Advertorial_Final.png`, type: "image/png", thumbnail: orderedImages[0] || resultImages[0], created_at: new Date() })}
