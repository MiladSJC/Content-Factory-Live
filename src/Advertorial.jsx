            ...
            >
                <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center z-10">
                    ...
                    <p className="mt-6 text-white text-sm font-bold tracking-wider uppercase animate-pulse">
                        {layoutMode === 'ai' ? 'AI Crafting Magazine Page' : 'Composing Layout'}
                    </p>
                    <p className="mt-2 text-gray-400 text-xs">
                        {layoutMode === 'ai' ? 'Generating layout with AI...' : 'AI is crafting your advertorial...'}
                    </p>
...
Design a sophisticated magazine page with high-quality visual hierarchy and premium typography.`;
                ...
                const response = await fetch('http://localhost:5001/generate-eblast', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        images: base64Images,
                        prompt: aiPrompt,
                        settings: { ...settings, aspectRatio: '2:3' },
                        is_live: true
                    })
                });
                ...
                if (response.ok) {
                    ...
                    setResultImages([result.image, result.image, result.image, result.image]);
                } else {
